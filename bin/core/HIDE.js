...
Main.main = function() {
	...
	js.Node.process.on("uncaughtException",function(err) {
		haxe.Log.trace(err,{ fileName : "Main.hx", lineNumber : 71, className : "Main", methodName : "main"});
		Main.window.show();
	});
	Main.sync = true;
	...
	window.addEventListener("load",function(e) {
		...
		processHelper.checkProcessInstalled("npm",["-v"],function(installed1) {
			haxe.Log.trace("npm installed " + (installed1 == null?"null":"" + installed1),{ fileName : "Main.hx", lineNumber : 174, className : "Main", methodName : "main"});
			if(installed1) processHelper.runProcess("npm",["list","-g","flambe"],null,function(stdout,stderr) {
				haxe.Log.trace("flambe installed " + Std.string(stdout.indexOf("(empty)") == -1),{ fileName : "Main.hx", lineNumber : 180, className : "Main", methodName : "main"});
			},function(code,stdout1,stderr1) {
				haxe.Log.trace("flambe installed " + Std.string(stdout1.indexOf("(empty)") == -1),{ fileName : "Main.hx", lineNumber : 184, className : "Main", methodName : "main"});
			});
		});
		processHelper.checkProcessInstalled("haxelib run lime",[],function(installed2) {
			haxe.Log.trace("lime installed " + (installed2 == null?"null":"" + installed2),{ fileName : "Main.hx", lineNumber : 193, className : "Main", methodName : "main"});
		});
		processHelper.checkProcessInstalled("git",["--version"],function(installed3) {
			haxe.Log.trace("git installed " + (installed3 == null?"null":"" + installed3),{ fileName : "Main.hx", lineNumber : 199, className : "Main", methodName : "main"});
		});
		Main.window.show();
	});
...
autoformat.HaxePrinterLoader.load = function() {
	...
	menu.BootstrapMenu.getMenu("Source",5).addMenuItem("Autoformat",1,function() {
		if(js.Node.require("path").extname(tabManagerInstance.getCurrentDocumentPath()) == ".hx") {
	...
	})($bind(tabManagerInstance,tabManagerInstance.openFileInNewTab),js.Node.require("path").join("core","config","autoformat.json")));
};
var bootstrap = {};
...
core.Completion.prototype = {
	...
	,getClassList: function() {
		...
		var mainClass = js.Node.require("path").basename(tabManagerInstance.getCurrentDocumentPath(),".hx");
			...
			js.Node.require("fs").stat(path[0],(function(path) {
				return function(err,stats) {
					if(stats.isDirectory()) {
						var filetreeInstance = filetree.FileTree.get();
						filetreeInstance.load(js.Node.require("path").basename(path[0]),path[0]);
...
core.HaxeHelper.updatePathToHaxe = function() {
	...
	if(pathToHaxeDirectory != null) {
		core.HaxeHelper.pathToHaxe = js.Node.require("path").join(pathToHaxeDirectory,core.HaxeHelper.pathToHaxe);
		core.HaxeHelper.pathToHaxelib = js.Node.require("path").join(pathToHaxeDirectory,core.HaxeHelper.pathToHaxelib);
	}
};
var haxe = {};
haxe.ds = {};
...
core.MenuCommands.add = function() {
	...
	})($bind(tabManagerInstance,tabManagerInstance.openFileInNewTab),js.Node.require("path").join("core","changes.md")));
	...
	})($bind(tabManagerInstance,tabManagerInstance.openFileInNewTab),js.Node.require("path").join("core","bindings.txt")));
	...
	nodejs.webkit.Window.get().on("close",$bind(tabManagerInstance,tabManagerInstance.saveAll));
	menu.BootstrapMenu.getMenu("Options").addMenuItem("Open settings",1,(function(f2,a12) {
		return function() {
			return f2(a12);
		};
	})($bind(tabManagerInstance,tabManagerInstance.openFileInNewTab),js.Node.require("path").join(watchers.SettingsWatcher.pathToFolder,"settings.json")));
	menu.BootstrapMenu.getMenu("Options").addMenuItem("Open stylesheet",1,function() {
		tabManagerInstance.openFileInNewTab(js.Node.require("path").join("core",watchers.SettingsWatcher.settings.theme));
	});
	var classpathWalker = parser.ClasspathWalker.get();
	...
	})($bind(tabManagerInstance,tabManagerInstance.openFileInNewTab),js.Node.require("path").join(watchers.SettingsWatcher.pathToFolder,"editor.json")));
	menu.BootstrapMenu.getMenu("Options").addMenuItem("Open templates folder",1,(function(f4,a14,a2) {
		return function() {
			return f4(a14,a2);
		};
	})($bind(fileTreeInstance,fileTreeInstance.load),"templates",js.Node.require("path").join("core","templates")));
	menu.BootstrapMenu.getMenu("Options").addMenuItem("Open localization file",1,(function(f5,a15) {
		return function() {
			return f5(a15);
		};
	})($bind(tabManagerInstance,tabManagerInstance.openFileInNewTab),js.Node.require("path").join("core","locale",watchers.SettingsWatcher.settings.locale)));
	menu.BootstrapMenu.getMenu("Options").addMenuItem("Open hotkey configuration file",1,(function(f6,a16) {
		return function() {
			return f6(a16);
		};
	})($bind(tabManagerInstance,tabManagerInstance.openFileInNewTab),js.Node.require("path").join(watchers.SettingsWatcher.pathToFolder,"hotkeys.json")));
...
filetree.FileTree.updateProjectMainHxml = function() {
	...
	switch(_g) {
	case 0:
		if(!noproject) main = js.Node.require("path").resolve(projectaccess.ProjectAccess.path,project.targetData[project.target].pathToHxml);
		break;
	case 2:
	...
	,load: function(projectName,path) {
		...
		var config = { path : path, listener : function(changeType,filePath,fileCurrentStat,filePreviousStat) {
			...
			case "create":
				...
				break;
			case "delete":
				if(js.Node.require("path").extname(filePath) != "") classpathWalker.removeFile(filePath);
				break;
			default:
			}
		}};
	...
	,createHaxeProject: function(data,target) {
		...
		js.node.Mkdirp.mkdirp(pathToSrc,function(err,made) {
			...
			if(data.projectPackage != "") {
				fullPackagePath = StringTools.replace(data.projectPackage,".",js.Node.require("path").sep);
				js.node.Mkdirp.mkdirpSync(js.Node.require("path").join(pathToSrc1,fullPackagePath));
			}
			var pathToMain;
			pathToMain = js.Node.require("path").join(pathToSrc1,fullPackagePath,"Main.hx");
			var tabManagerInstance = tabmanager.TabManager.get();
			var fileTemplate = { };
			...
			js.Node.require("fs").writeFile(pathToMain,templateCode,null,function(error) {
			...
			while(_g2 < _g1) {
				...
				var options = { };
				options.encoding = "utf8";
				var templateCode1 = js.Node.require("fs").readFileSync(js.Node.require("path").join(pathToProjectTemplates,filenames[i] + ".tpl"),options);
				var pathToFile;
				switch(i) {
				case 0:
				...
				js.Node.require("fs").writeFileSync(js.Node.require("path").join(pathToProject,targetData.pathToHxml),templateCode2,"utf8");
...
openflproject.OpenFLProject.prototype = {
	createOpenFLProject: function(data,sample) {
		...
		openflproject.CreateOpenFLProject.createOpenFLProject(params,data.projectLocation,function() {
			...
			var tabManagerInstance = tabmanager.TabManager.get();
			tabManagerInstance.openFileInNewTab(js.Node.require("path").join(pathToProject,"Source","Main.hx"));
		});
	}
...
outline.OutlineParser.prototype = {
	parse: function(data,path) {
		...
		haxe.Log.trace(usingSmartSort,{ fileName : "OutlineParser.hx", lineNumber : 179, className : "outline.OutlineParser", methodName : "parse"});
		if(usingSmartSort) {
			var nameA;
			var nameB;
		...
		while(varFields.length != 0 || methodFields.length != 0) {
			if(usingSmartSort) {
				if(varFields.length != 0) {
					...
					if(varFields.length == 0) {
						parentIndex = 0;
						haxe.Log.trace("length == 0",{ fileName : "OutlineParser.hx", lineNumber : 214, className : "outline.OutlineParser", methodName : "parse"});
					}
	...
	,getFunctionDeclarations: function(data) {
		...
		eregFunctionWithParameters.map(data,function(ereg2) {
			...
			if(name != "") {
				...
				var functionBodyLength = leftBraces[0].length;
				var unClosedBraces = 0;
				var leftBrace;
				var rightBrace;
				haxe.Log.trace("StartBodyLength",{ fileName : "OutlineParser.hx", lineNumber : 334, className : "outline.OutlineParser", methodName : "getFunctionDeclarations", customParams : [functionBodyLength]});
				var _g1 = 1;
				...
				while(_g1 < _g) {
					...
					if(rightBraces.length == 1) {
						...
						haxe.Log.trace("noRightBrace",{ fileName : "OutlineParser.hx", lineNumber : 346, className : "outline.OutlineParser", methodName : "getFunctionDeclarations", customParams : ["length:",rightBraces[0].length,unClosedBraces,"functionBodyLength:  ",functionBodyLength,"** \n",rightBraces[0]]});
						continue;
					}
					var _g3 = 0;
					...
					while(_g3 < _g2) {
						...
						if(j != 0) unClosedBraces--; else functionBodyLength++;
						functionBodyLength += rightBrace.length;
						haxe.Log.trace("rightBrace",{ fileName : "OutlineParser.hx", lineNumber : 358, className : "outline.OutlineParser", methodName : "getFunctionDeclarations", customParams : ["length:",rightBrace.length,unClosedBraces,functionBodyLength,"** \n",rightBrace]});
						if(unClosedBraces == 0) {
							haxe.Log.trace(name,{ fileName : "OutlineParser.hx", lineNumber : 362, className : "outline.OutlineParser", methodName : "getFunctionDeclarations", customParams : ["foundEnd"]});
							break;
						}
					}
					if(unClosedBraces == 0) {
						haxe.Log.trace(name,{ fileName : "OutlineParser.hx", lineNumber : 370, className : "outline.OutlineParser", methodName : "getFunctionDeclarations", customParams : ["foundEnd"]});
						break;
					}
				}
				haxe.Log.trace(pos.pos,{ fileName : "OutlineParser.hx", lineNumber : 377, className : "outline.OutlineParser", methodName : "getFunctionDeclarations", customParams : [pos.len,functionBodyLength]});
	...
	,addFile: function(path,std) {
		...
		if(!watchers.SettingsWatcher.isItemInIgnoreList(path) && !projectaccess.ProjectAccess.isItemInIgnoreList(path)) {
			...
			if(projectaccess.ProjectAccess.path != null && (core.Utils.os == 0 || !std)) {
				...
				if(this.getFileIndex(relativePath,list) == -1) list.push({ path : relativePath, directory : this.getFileDirectory(relativePath), displayText : completionInstance.processDisplayText(relativePath), filename : js.Node.require("path").basename(relativePath)});
			} else if(this.getFileIndex(path,list) == -1) list.push({ path : path, directory : this.getFileDirectory(path), displayText : completionInstance.processDisplayText(path), filename : js.Node.require("path").basename(path)});
		}
	}
...
pluginloader.PluginManager.prototype = {
						...
						js.Node.require("fs").stat(pathToFolder,(function(item) {
							return function(error1,stat) {
								if(error1 != null) {
									...
									var pluginName = StringTools.replace(pathToPlugin,js.Node.require("path").sep,".");
									if(stat.isDirectory()) _g1.readDir(path,js.Node.require("path").join(pathToPlugin,item[0]),onLoad); else if(item[0] == "plugin.hxml" && !Lambda.has(_g1.inactivePlugins,pluginName)) {
										var levels = "";
										var _g3 = 0;
	...
	,compilePlugin: function(name,pathToPlugin,onSuccess,onFailed) {
		...
		js.Node.require("fs").exists(pathToBin,function(exists) {
			if(exists) _g.startPluginCompilation(name,pathToPlugin,onSuccess,onFailed); else js.Node.require("fs").mkdir(pathToBin,null,function(error) {
...
projectaccess.ProjectOptions.prototype = {
	...
	,createOptionsForMultipleHxmlProjects: function() {
		...
		this.pathToHxmlInput.onchange = function(e) {
			if(js.Node.require("fs").existsSync(_g.pathToHxmlInput.value)) {
				var project = projectaccess.ProjectAccess.currentProject;
		...
		editButton.onclick = function(e2) {
			var tabManagerInstance = tabmanager.TabManager.get();
			tabManagerInstance.openFileInNewTab(js.Node.require("path").resolve(projectaccess.ProjectAccess.path,_g.pathToHxmlInput.value));
		};
...
tabmanager.TabManager.prototype = {
			...
			classpathWalker.getProjectClasspaths(projectaccess.ProjectAccess.currentProject,function(classpathAndLibs) {
				...
				while(_g3 < classpaths.length) {
					...
					if(StringTools.startsWith(pathToFile,classpath)) {
						var dirname = js.Node.require("path").dirname(pathToFile);
						var relativePath = js.Node.require("path").relative(classpath,dirname);
						var fullPackagePath = StringTools.replace(relativePath,js.Node.require("path").sep,".");
	...
	,createNewFile: function(path) {
		...
		case ".hx":
			...
			var options = { };
			options.encoding = "utf8";
			var pathToTemplate = js.Node.require("path").join("core","templates","New.tpl");
			var templateCode = js.Node.require("fs").readFileSync(pathToTemplate,options);
			this.resolvePackage(path,function(filePackage) {
				var data = { };
				data.name = js.Node.require("path").basename(name,extname);
		...
		case ".hxml":
			var options1 = { };
			options1.encoding = "utf8";
			var pathToTemplate1 = js.Node.require("path").join("core","templates","build.tpl");
	...
	,selectDoc: function(path) {
		...
		if(found) {
			...
			if(projectaccess.ProjectAccess.path != null) project.activeFile = js.Node.require("path").relative(projectaccess.ProjectAccess.path,this.selectedPath);
